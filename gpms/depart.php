...
<form method="POST" name="c_del" action="?pid=depart"></form>

<form method="POST" action="?pid=depart&sub=delconf">
  <p>
...
$insert_query = "DELETE FROM `".$tbl."depart` WHERE `id` = ".$depart_id." and `user_id`= '".$_SESSION['GPMA_key']."' LIMIT 1";
...
mysql_select_db($database_connection, $connection);
$insert_query = sprintf("INSERT INTO `".$tbl."depart` (name, `user_id`) VALUES ( %s, %s)",
...
if ($sub=="" || $sub=="save"){
mysql_select_db($database_connection, $connection);
$depart_query="SELECT * FROM `".$tbl."depart` WHERE `user_id` = '".$_SESSION['GPMA_key']."'  ORDER BY `name` ASC"; 
...
$s_no = $s_no + 1;
echo'<form method="POST" action="?pid=depart&sub=del">
  <input type="hidden" name="depart_id" value="'.$depart_id.'" size="20">
  <input type="hidden" name="name" value="'.$depart_name.'" size="20">
<input type="hidden" name="action" value="del_depart" size="20">
