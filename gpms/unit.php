...
$unit_id=@$_POST['depart_id'];
$unit_name=@$_POST['name'];
$depart_action=@$_POST['action'];

if ($sub=="del"){
echo'<p><font face="verdana" size="2" color="#FF0000">Are You Sure to delete <b>'.$unit_name.' ?
<form method="POST" name="c_del" action="?pid=unit"></form>

<form method="POST" action="?pid=unit&sub=delconf">
  <p>
  <input type="hidden" name="depart_id" value="'.$unit_id.'" size="20">
  <input type="hidden" name="name" value="'.$unit_name.'" size="20">
...
$insert_query = "DELETE FROM `".$tbl."unit` WHERE `id` = ".$unit_id." and `user_id`= '".$_SESSION['GPMA_key']."' LIMIT 1";
							
								
	$result = mysql_query($insert_query, $connection) or die(
'<p><font face="verdana" size="2" color="#FF0000"><b>Error!</b><br>'.mysql_error().'<br>
<a href="?pid=unit">Click here to Continue</a>');
...
if ($sub=="save"){
if ( $depart_action == 'unit'){

mysql_select_db($database_connection, $connection);
$insert_query = sprintf("INSERT INTO `".$tbl."unit` (name, `user_id`) VALUES ( %s, %s)",
							sanitize($unit_name, "text"),
...
if ($sub=="" || $sub=="save"){
mysql_select_db($database_connection, $connection);
$depart_query="SELECT * FROM `".$tbl."unit` WHERE `user_id`= '".$_SESSION['GPMA_key']."' ORDER BY `name` ASC"; 
...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#0099FF" width="100%" id="AutoNumber1" bgcolor="#0099FF">
  <tr>
    ...
    <font face="Verdana" size="2" color="#FFFFFF">&nbsp;Unit</font></b></td>
...
while ($depart_row=mysql_fetch_array($depart_result)){ 
	$unit_name=$depart_row['name']; 
	$unit_id=$depart_row['id']; 
$s_no = $s_no + 1;
echo'
  <tr><form method="POST" action="?pid=unit&sub=del">
  <input type="hidden" name="depart_id" value="'.$unit_id.'" size="20">
  <input type="hidden" name="name" value="'.$unit_name.'" size="20">
<input type="hidden" name="action" value="del_depart" size="20">
