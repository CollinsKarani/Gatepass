...
$depart_cat=@$_POST['p_cat'];
$depart_action=@$_POST['action'];


if ($sub=="del"){
echo'<p><font face="verdana" size="2" color="#FF0000">Are You Sure to delete <b>'.$depart_name.' ?
<form method="POST" name="c_del" action="?pid=product"></form>

<form method="POST" action="?pid=product&sub=delconf">
  <p>
...
$insert_query = "DELETE FROM `".$tbl."product` WHERE `id` = ".$depart_id." and `user_id`= '".$_SESSION['GPMA_key']."' LIMIT 1";
							
								
	$result = mysql_query($insert_query, $connection) or die(
'<p><font face="verdana" size="2" color="#FF0000"><b>Error!</b><br>'.mysql_error().'<br>
<a href="?pid=product">Click here to Continue</a>');
...
mysql_select_db($database_connection, $connection);
$insert_query = sprintf("INSERT INTO `".$tbl."product` (name, cat, `user_id`) VALUES ( %s,%s, %s)",
							sanitize($depart_name, "text"),
							sanitize($depart_cat, "text"),
...
if ($sub=="" || $sub=="save"){
mysql_select_db($database_connection, $connection);
$depart_query="SELECT * FROM `".$tbl."product` WHERE `user_id` = '".$_SESSION['GPMA_key']."'  ORDER BY `name` ASC"; 
...
<table border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#0099FF" width="100%" id="AutoNumber1" bgcolor="#0099FF">
  <tr>
    ...
    <font face="Verdana" size="2" color="#FFFFFF">&nbsp;Product</font></b></td>
    <td width="36%" bordercolor="#000080" bgcolor="#0099FF"><b>
...
while ($depart_row=mysql_fetch_array($depart_result)){ 
	...
	$depart_cat=$depart_row['cat']; 
	$depart_id=$depart_row['id']; 
$s_no = $s_no + 1;
echo'<form method="POST" action="?pid=product&sub=del">
  <input type="hidden" name="depart_id" value="'.$depart_id.'" size="20">
  <input type="hidden" name="name" value="'.$depart_name.'" size="20">
<input type="hidden" name="action" value="del_depart" size="20">

  <tr>
    <td width="5%" bordercolor="#000080" bgcolor="#FFFFFF"> <font face="Verdana" size="2" >
'.$s_no.'</td>
